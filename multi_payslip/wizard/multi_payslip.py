...
#         multi_employees = self.env['multi.employee.wiz'].search([('active', '=', True)])
        action = []
        for emp_id in self.employee_ids:
            emp_contract = self.env['hr.contract'].get_employee_active_contract(emp_id)
            if emp_contract['contract_id'] == False:
                raise Warning(_('Please Create Contract for employee = %s.') % emp_id.employee_id.name)
            now = datetime.now()
